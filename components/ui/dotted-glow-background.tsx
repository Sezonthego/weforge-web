...
type DottedGlowBackgroundProps = {
  ...
  /** optional CSS variable name for light dot color (e.g. --color-brand-cocoa) */
  colorLightVar?: string;
  /** optional CSS variable name for dark dot color (e.g. --color-brand-ivory) */
  colorDarkVar?: string;
  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      if (backgroundOpacity > 0) {
        ...
        grad.addColorStop(0, "rgba(32,21,21,0)");
        grad.addColorStop(
          1,
          `rgba(32,21,21,${Math.min(Math.max(backgroundOpacity, 0), 1)})`,
        );
